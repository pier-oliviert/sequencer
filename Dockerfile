...
# -- Solver
FROM build-base as build-solver

# Copy the go source
COPY cmd/solver/main.go cmd/solver/main.go
COPY api/ api/
COPY pkg/ pkg/
...
# the docker BUILDPLATFORM arg will be linux/arm64 when for Apple x86 it will be linux/amd64. Therefore,
# by leaving it empty we can ensure that the container and binary shipped on it will have the same platform.
RUN CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build -a -o solver cmd/solver/main.go

FROM alpine:3 as solver
...
WORKDIR /
COPY --from=build-solver /workspace/solver .

# USER user:user
ENTRYPOINT ["/solver"]

# -- Github
FROM build-base as build-gh

# Copy the go source
COPY cmd/github/main.go cmd/github/main.go
COPY api/ api/
COPY internal/ internal/
COPY pkg/ pkg/
...
# the docker BUILDPLATFORM arg will be linux/arm64 when for Apple x86 it will be linux/amd64. Therefore,
# by leaving it empty we can ensure that the container and binary shipped on it will have the same platform.
RUN CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build -a -o sequencer-github cmd/github/main.go

FROM alpine:3 as github
...
WORKDIR /
COPY --from=build-gh /workspace/sequencer-github .

# USER user:user
ENTRYPOINT ["/sequencer-github"]
