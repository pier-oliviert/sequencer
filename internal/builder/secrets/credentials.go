...
var ErrCredentialsIncomplete = errors.New("E#TODO: couldn't create a valid credential")
...
func ReadCredentialsFromDir(path string, c *buildConfig.Credentials) (*Credentials, error) {
	...
	case buildConfig.KeyPair:
		...
		if err != nil {
			return nil, fmt.Errorf("E#TODO: error while reading the content of the file (%s/accessKey) -> %w", filepath.Join(path, *c.Name), err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("E#TODO: error while reading the content of the file (%s/secretToken) -> %w", filepath.Join(path, *c.Name), err)
		}
